            ...
            <div class="col-12 row q-mb-sm">
              <div class="q-ml-md-auto q-mx-xs-auto q-mx-md-none flex justify-lg-end">
                <q-btn @click="go('/favourite-item')" outline round color="dark" class="q-mr-sm" icon="favorite_border">
                  <q-badge color="red" text-color="white"  floating>{{favouriteItem}}</q-badge>
                  <q-tooltip anchor="center right" self="center left">
                    Favourite items
                  </q-tooltip>
                </q-btn>
                <q-btn outline round @click="go('/cart')" color="dark" class="q-ml-lg q-mr-sm" icon="shopping_cart">
                  <q-badge color="red" text-color="white"  floating>{{cartItem}}</q-badge>
      ...
      <div class="row col-11 q-mx-auto text-black">
        ...
        >
            <div class="flex q-mr-auto">
              ...
              <q-tab label="MEN" @click="go('/men-products/all')" name="/men-products" class="q-px-md" >
                <!--<q-menu
                  class="full-width"
                  transition-show="jump-down"
                  transition-hide="jump-up"
                >
                  <div @click="go('/men-products/all')" class="flex q-pa-md justify-center main-title text-h6 link-rt text-weight-bold items-center">
                    MEN FASHION
                  </div>
                  ...
                  <div class="flex q-mx-sm justify-center">
                    ...
                    <div class="col-12 row" style="width: 400px">
                      <div v-for="item in this.cate" @click="go(`/men-products/${item.categoryName}`)" class="link-rt col-4 text-center">
                        {{item.categoryName}}
                      </div>
                    </div>
                  <div class="full-width q-mt-md">
                    <q-separator  inset/>
                    <div @click="go('/men-products/all')" class="q-my-sm text-center text-subtitle2 link-rt text-weight-bold">See all...</div>
                  </div>
                  </div>
                </q-menu>-->
              </q-tab>
              <q-tab label="WOMEN" @click="go('/women-products/all')" class="q-px-md" >
                <!--<q-menu
                  class="full-width"
                  transition-show="jump-down"
                  transition-hide="jump-up"
                >
                  <div @click="go('/women-products/all')" class="flex q-pa-md justify-center main-title text-h6 link-rt text-weight-bold items-center">
                    WOMEN FASHION
                  </div>
                  <q-separator class="q-mb-md" inset/>
                  <div class="flex q-mx-sm justify-center">
                   <div class="col-12 row" style="width: 400px">
                      <div v-for="item in this.cate" @click="go(`/women-products/${item.categoryName}`)" class="link-rt col-4 text-center">
                        {{item.categoryName}}
                      </div>
                    </div>
                    <div class="full-width q-mt-md">
                      <q-separator  inset/>
                      <div @click="go(`/women-products/all`)" class="q-my-sm text-center text-subtitle2 link-rt text-weight-bold">See all...</div>
                    </div>
                  </div>
                </q-menu>-->
              </q-tab>
...
export default {
  ...
  computed: {
    ...
    ...mapState(
      "product", ['favouriteItem','cartItem']
    ),
    ...mapState(
      'user', ['user','isLogged']
    )
  ...
  watch: { //favouriteItem
    favouriteItem() {
      //console.log("favou", this.favouriteItem)
    },
    notify() {
      if (this.notify) {
        this.$q.notify({
